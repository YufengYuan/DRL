...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        for i in idxes:
            ...
            dones.append(done)
        #TODO: better way to ensure those tensor have correct shape
        return torch.tensor(np.array(obses_t), dtype=torch.float32, device=self.device), \
                ...
                ensure2d(torch.tensor(np.array(rewards), dtype=torch.float32, device=self.device)), \
                ensure2d(torch.tensor(np.array(obses_tp1), dtype=torch.float32, device=self.device)), \
                ensure2d(torch.tensor(np.array(dones), dtype=torch.float32, device=self.device)), \
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def update_priorities(self, idxes, priorities, alpha=1):
        ...
        for idx, priority in zip(idxes, priorities):
            ...
            self._it_sum[idx] = priority ** alpha
